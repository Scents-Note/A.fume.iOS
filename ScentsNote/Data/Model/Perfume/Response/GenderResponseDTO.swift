...
extension GenderResponseDTO {
  func toDomain() -> [Gender] {
    ...
    genders.append(Gender(gender: "남성",
                          percent: percents[0],
                          color: .SNDarkBeige1,
                          isAccent: percents[0] == max && percents[0] != 0))
    genders.append(Gender(gender: "여성",
                          percent: percents[2],
                          color: .pointBeige,
                          isAccent: percents[2] == max && percents[2] != 0))
    genders.append(Gender(gender: "중성",
                          percent: percents[1],
                          color: .SNLightBeige239,
                          isAccent: percents[1] == max && percents[1] != 0))
    return genders
  }
}
